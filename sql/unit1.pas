...
if a3=1 then
  ...
  if ((sender as TEdit).Name = 'Edit1') then
     begin
       SQLQuery1.SQL.Add('SELECT members.firstname AS "Имя", members.lastname AS "Фамилия", books.title AS "Книга", borrowings.borrowdate AS "Когда взял", borrowings.returndate AS "Когда отдать" FROM borrowings JOIN books ON borrowings.bookid=books.bookid JOIN members ON borrowings.memberid=members.memberid WHERE firstname LIKE :fn;');
       SQLQuery1.params.parambyname('fn').asstring := edit1.Text + '%';
     end;
    if ((sender as TEdit).Name = 'Edit2') then
     begin
       SQLQuery1.SQL.Add('SELECT members.firstname AS "Имя", members.lastname AS "Фамилия", books.title AS "Книга", borrowings.borrowdate AS "Когда взял", borrowings.returndate AS "Когда отдать" FROM borrowings JOIN books ON borrowings.bookid=books.bookid JOIN members ON borrowings.memberid=members.memberid WHERE lastname LIKE :ln;');
       SQLQuery1.params.parambyname('ln').asstring := edit2.Text+'%';
     end;
  SQLQuery1.Open;
end;
